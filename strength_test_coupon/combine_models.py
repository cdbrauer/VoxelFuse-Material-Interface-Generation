...
if __name__=='__main__':
    ...
    files = ['output-control', 'output-blur', 'output-dither', 'output-dither2', 'output-gyroid', 'output-schwarzp', 'output-schwarzd', 'output-plus', 'output-cross', 'output-zz5']

    model = VoxelModel.emptyLike(cube(1))
    for i in range(len(files)):
        new_model = VoxelModel.openVF('stl_files_v2_combined/' + files[i])
    ...
    if export:
        for m in range(1, len(model.materials)):
            ...
            mesh.export(('stl_files_v2_combined/' + couponStandard + '_mat_' + str(m) + '_' + str(model.materials[m, 2]) + '.stl'))
